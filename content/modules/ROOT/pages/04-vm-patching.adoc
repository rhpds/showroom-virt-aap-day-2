...
NOTE: To ensure the virtual machines can access the required repositories for updates, the environment is configured to use the redhatone.vm_management.rhsm_subscription role for managing subscriptions. Additionally, a Vault Credential is employed to securely handle sensitive data, such as activation keys and Satellite server credentials, during the automation process. These configurations have already been created for you and require no additional setup.
